...
const BatchDetail = ({ batch, onBack }) => {
  ...
  const [bulkProgressPdfLoading, setBulkProgressPdfLoading] = useState(false);
  const [bulkProgressPdfCount, setBulkProgressPdfCount] = useState({ done: 0, total: 0 });
  ...
  const toPercent = (obtained, total) => {
    const obtainedNum = parseNumeric(obtained);
  ...
  const estimateAveragePercent = (tests = [], type = 'daily') => {
    if (!tests.length) return null;
    const values = tests
      .map((test) => {
        if (type === 'daily') {
          return toPercent(test.marks, test.subject_total_marks ?? test.test_total_marks)
            ?? parseNumeric(test.marks);
        }
        return toPercent(test.total_marks, test.test_total_marks)
          ?? parseNumeric(test.total_marks);
      })
  ...
  const toSafeFilePart = (value, fallback = 'Student') => {
  ...
  const handleDownloadAllProgressReportsCombined = async () => {
    if (!students.length || bulkProgressPdfLoading) return;

    const shouldProceed = window.confirm(
      `Generate one combined progress PDF for all ${students.length} students in this batch? This may take some time.`
    );
    if (!shouldProceed) return;

    setBulkProgressPdfLoading(true);
    setBulkProgressPdfCount({ done: 0, total: students.length });
    ...
    try {
      ...
      for (let i = 0; i < students.length; i += 1) {
        ...
        pdf.setFontSize(17);
        pdf.text('Student Progress Report', marginX, 54);

        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(11);
        pdf.text(`Batch: ${batch.batch_name || 'N/A'}`, marginX, 74);
        pdf.text(`Generated: ${new Date().toLocaleString('en-IN')}`, marginX, 90);

        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(13);
        pdf.text(`${st.name || 'N/A'} (${sid || 'N/A'})`, marginX, 120);

        let y = 142;
        let analysis = null;

        try {
          const analysisRes = await authFetch(`${API_BASE}/api/analysis/individual/${encodeURIComponent(sid)}`);
          if (analysisRes.ok) {
            analysis = await analysisRes.json();
          }
        } catch (err) {
          console.error(`Failed to fetch analysis for ${sid}:`, err);
        }

        if (!analysis) {
          pdf.setFont('helvetica', 'normal');
          pdf.setFontSize(11);
          pdf.text('Unable to load analysis data for this student.', marginX, y);
          y += 18;
          pdf.text('Please open the student profile and export manually.', marginX, y);
          setBulkProgressPdfCount({ done: i + 1, total: students.length });
          continue;
        }

        const dailyTests = analysis.daily_tests || [];
        const mockTests = analysis.mock_tests || [];
        const dailyAvg = estimateAveragePercent(dailyTests, 'daily');
        const mockAvg = estimateAveragePercent(mockTests, 'mock');

        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(11);
        const summaryLines = [
          `Grade: ${analysis.student?.grade || st.grade || 'N/A'}`,
          `Course / Branch: ${(analysis.student?.course || st.course || 'N/A')} / ${(analysis.student?.branch || st.branch || 'N/A')}`,
          `Daily Tests Recorded: ${dailyTests.length}`,
          `Mock Tests Recorded: ${mockTests.length}`,
          `Estimated Daily Average: ${dailyAvg !== null ? `${dailyAvg}%` : 'N/A'}`,
          `Estimated Mock Average: ${mockAvg !== null ? `${mockAvg}%` : 'N/A'}`
        ];
        ...
        y += 8;
        pdf.setFont('helvetica', 'bold');
        pdf.text('Recent Daily Tests (latest 5):', marginX, y);
        y += 16;
        pdf.setFont('helvetica', 'normal');

        const recentDaily = [...dailyTests]
          .sort((a, b) => new Date(b.test_date || 0) - new Date(a.test_date || 0))
          .slice(0, 5);

        if (!recentDaily.length) {
          pdf.text('No daily test records.', marginX, y);
          y += 16;
        } else {
          recentDaily.forEach((test) => {
            const dateText = test.test_date ? new Date(test.test_date).toLocaleDateString('en-IN') : 'N/A';
            const line = `• ${dateText} | ${test.subject || 'N/A'} | ${test.unit_name || 'N/A'} | Marks: ${test.marks ?? 'N/A'}`;
        ...
        y += 8;
        pdf.setFont('helvetica', 'bold');
        pdf.text('Recent Mock Tests (latest 5):', marginX, y);
        y += 16;
        pdf.setFont('helvetica', 'normal');

        const recentMock = [...mockTests]
          .sort((a, b) => new Date(b.test_date || 0) - new Date(a.test_date || 0))
          .slice(0, 5);

        if (!recentMock.length) {
          pdf.text('No mock test records.', marginX, y);
        } else {
          recentMock.forEach((test) => {
            const dateText = test.test_date ? new Date(test.test_date).toLocaleDateString('en-IN') : 'N/A';
            const line = `• ${dateText} | Total: ${test.total_marks ?? 'N/A'} / ${test.test_total_marks ?? 'N/A'} | Class Avg: ${test.class_avg_total ?? 'N/A'} | Top: ${test.top_score_total ?? 'N/A'}`;
            const wrapped = pdf.splitTextToSize(line, contentWidth);
            pdf.text(wrapped, marginX, y);
            y += wrapped.length * 14;
          });
      ...
      const safeBatchName = (batch.batch_name || 'Batch').replace(/\s+/g, '_');
      const dateStr = new Date().toISOString().split('T')[0];
      pdf.save(`${safeBatchName}_All_Students_Progress_${dateStr}.pdf`);
    } catch (err) {
      console.error('Failed to generate combined progress PDF:', err);
      alert('Failed to generate combined progress PDF. Please try again.');
    } finally {
      setBulkProgressPdfLoading(false);
      setBulkProgressPdfCount({ done: 0, total: 0 });
    }
  };

  const handleDownloadAllProgressReportsZip = async () => {
    if (!students.length || bulkProgressPdfLoading) return;

    const shouldProceed = window.confirm(
      `Generate separate progress PDFs for all ${students.length} students and download as a ZIP? This may take some time.`
    );
    if (!shouldProceed) return;
    ...
    try {
      ...
      for (let i = 0; i < students.length; i += 1) {
        ...
        if (!analysis) {
          ...
          const fallbackName = `${toSafeFilePart(st.name)}_${toSafeFilePart(sid, 'NA')}_Progress_Report.pdf`;
          zip.file(fallbackName, pdf.output('arraybuffer'));
          setBulkProgressPdfCount({ done: i + 1, total: students.length });
          continue;
        }
        ...
        const studentFileName = `${toSafeFilePart(st.name)}_${toSafeFilePart(sid, 'NA')}_Progress_Report.pdf`;
        zip.file(studentFileName, pdf.output('arraybuffer'));
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${safeBatchName}_All_Students_Progress_${dateStr}.zip`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url);
    } catch (err) {
      console.error('Failed to generate progress ZIP:', err);
      alert('Failed to generate progress ZIP. Please try again.');
    } finally {
      setBulkProgressPdfLoading(false);
      setBulkProgressPdfCount({ done: 0, total: 0 });
    }
      ...
      {activeTab === 'students' && (
        <>
          ...
          <div className="management-buttons">
            ...
            <button className="btn btn-report" onClick={handleDownloadAllProgressReportsCombined} disabled={bulkProgressPdfLoading || students.length === 0}>
              {bulkProgressPdfLoading
                ? `⏳ Generating Progress PDFs... (${bulkProgressPdfCount.done}/${bulkProgressPdfCount.total})`
                : '📄 Download Combined Progress PDF'}
            </button>
            <button className="btn btn-report" onClick={handleDownloadAllProgressReportsZip} disabled={bulkProgressPdfLoading || students.length === 0}>
              {bulkProgressPdfLoading
                ? `⏳ Generating Progress PDFs... (${bulkProgressPdfCount.done}/${bulkProgressPdfCount.total})`
                : '🗜️ Download ZIP (Per-Student PDFs)'}
            </button>
          </div>
